...
export default function DialogAdminPage() {
  ...
  // Загрузка настроек автоответчика
  const loadAutoResponderSettings = async () => {
    try {
      const response = await fetch('/api/dialog/admin/auto-responder')
      if (!response.ok) {
        throw new Error('Request failed')
      }
      const data = await response.json()
      ...
      console.error('[Dialog Admin] Failed to load auto-responder settings:', error)
      // Не показываем ошибку, так как это не критично
    }
  }

  // Сохранение настроек автоответчика
  const saveAutoResponderSettings = async () => {
    ...
    try {
      ...
      const data = await response.json()
      if (!data.success) {
        throw new Error(data.error || 'Failed to save auto-responder settings')
      }

      showSuccess('Настройки автоответчика сохранены')
    } catch (error) {
      console.error('[Dialog Admin] Failed to save auto-responder settings:', error)
      showError('Не удалось сохранить настройки автоответчика')
    } finally {
    ...
    <AuthGuard>
      <div className="pt-24 pb-20 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
        ...
        {isLoading ? (
          ...
          <>
            {/* Автоответчик - отдельная карточка сверху */}
            <SectionCard title="Автоответчик" className="mb-6">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 text-[#d7a13a] border-gray-300 rounded focus:ring-[#d7a13a] focus:ring-2"
                    />
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                      Включить режим автоответчика
                    </span>
                  </label>
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    disabled={isSavingAutoResponder}
                    className={cn(
                      'inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#d7a13a] text-white text-sm hover:bg-[#c18c28]',
                      isSavingAutoResponder && 'opacity-60 cursor-not-allowed'
                    )}
                  >
                    {isSavingAutoResponder ? 'Сохранение...' : 'Сохранить настройки'}
                  </button>
                  
                  {autoResponder.enabled && (
                    <div className="text-sm text-amber-600 dark:text-amber-400 flex items-center gap-1">
                      ⚠️ Режим автоответчика включен - бот будет отвечать только заданным текстом
                    </div>
                  )}
                </div>
              </div>
